...
MyObject::MyObject(int s, double frequency, int octaves) : fastNoise(s) {
...
void MyObject::New(const FunctionCallbackInfo<Value>& args) {
  ...
  if (args.IsConstructCall()) {
    ...
    MyObject* obj = new MyObject(seed, frequency, octaves);
    obj->Wrap(args.This());
...
void MyObject::in2D(const FunctionCallbackInfo<Value>& args) {
  Isolate* isolate = args.GetIsolate();
